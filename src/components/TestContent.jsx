...
const TestContent = () => {
  ...
  // Log student and category to verify they are passed correctly
  useEffect(() => {
  ...
  // Generate PDF with test results
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    ...
    doc.text(`Natija: ${score >= passingScore ? 'Passed' : 'Failed'}`, 10, 60);
            ...
            <div className='flex flex-col items-start'>
              {questions[currentQuestion]?.choices.map((choice, idx) => (
                <button 
                  ...
                  className={`text-left text-lg hover:text-[green] ${answers[questions[currentQuestion]?.id]?.selectedAnswer === choice ? 'bg-green-500' : ''}`} 
                  disabled={Boolean(answers[questions[currentQuestion]?.id])} // Using Boolean for clarity
                >
                  {`${idx + 1}. ${choice}`}
                </button>
              ))}
